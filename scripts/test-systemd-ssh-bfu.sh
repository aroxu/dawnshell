...
if [[ "${BFU_REQUIRE_HARDWARE_CODEC:-0}" == "1" ]]; then
  ...
  grep -Fq '"worker_state":"ready"' <<<"$locked_codec_result"
  grep -Fq '"transport":"inherited_memfd_eventfd"' <<<"$locked_codec_result"
  grep -Fq '"public_listener":false' <<<"$locked_codec_result"
  grep -Fq '"software_fallback":false' <<<"$locked_codec_result"
  ...
  if [[ "${BFU_REQUIRE_CODEC_PERFORMANCE:-0}" == "1" ]]; then
    ...
    grep -Fq 'inherited memfd/eventfd transport' <<<"$locked_codec_result"
    grep -Fq 'malformed-input isolation' <<<"$locked_codec_result"
    grep -Fq 'concurrent workers' <<<"$locked_codec_result"
  ...
  echo "PASS: BFU private codec worker and hardware self-test succeeded."
fi
...
if [[ "${BFU_REQUIRE_HARDWARE_CODEC:-0}" == "1" ]]; then
  echo "Starting a fresh private codec worker after USER_UNLOCKED..."
  # shellcheck disable=SC2029
  unlocked_codec_result="$(ssh "${ssh_args[@]}" \
    "$ssh_user@$BFU_PHONE_HOST" "$codec_command")"
  printf 'AFU codec result:\n%s\n' "$unlocked_codec_result"
  grep -Fq '"worker_state":"ready"' <<<"$unlocked_codec_result"
  grep -Fq '"transport":"inherited_memfd_eventfd"' <<<"$unlocked_codec_result"
  grep -Fq '"public_listener":false' <<<"$unlocked_codec_result"
  grep -Fq '"software_fallback":false' <<<"$unlocked_codec_result"
  ...
  if [[ "${BFU_REQUIRE_CODEC_PERFORMANCE:-0}" == "1" ]]; then
    ...
    grep -Fq 'malformed-input isolation' <<<"$unlocked_codec_result"
    grep -Fq 'concurrent workers' <<<"$unlocked_codec_result"
  ...
  echo "PASS: AFU private codec worker and hardware self-test succeeded."
fi
