...
- `main`: 성공한 빌드마다 공개 debug 키로 서명한 새 일반 Release를 만들고
  Latest로 표시합니다. 태그는
  `continuous-<버전>-<실행 번호>-<재시도 번호>-<커밋>` 형식이며,
  기존 Release나 첨부 파일을 덮어쓰지 않습니다.
...
`main` 빌드 Release는 저장소의 공개 개발 키로 서명되므로 같은 키로 서명한 APK끼리만
